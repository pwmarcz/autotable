...
export class Setup {
  ...
  private tileIndex(i: number, conditions: Conditions): number | null {
    ...
    if (i>135) tileIndex = i - 96; //LG Edit: custom tile indexes for last 8 flower tiles that will get index 40-47
  ...
  deal(seat: number, gameType: GameType, dealType: DealType): void {
    ...
    const roll = 9;//Math.floor(Math.random() * 4)+ 1 + 4; //LG debug, always the same roll during testing
